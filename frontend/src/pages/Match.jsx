...
function getScoreMeta(score) {
  ...
  if (score >= 60) return { color: 'var(--primary)',  label: 'Good Match 👍',      grade: 'B' }
...
export default function Match() {
  ...
  async function handleMatch(e) {
    ...
    if (!rid || rid < 1) { setError('Please select a valid Resume.'); return }
    if (!jid || jid < 1) { setError('Please select a valid Job.'); return }
  ...
  return (
    <div className="fade-up">
      <div className="page-header">
        <div>
          <h2>Analytics — Match Score</h2>
          <p>Compute TF-IDF cosine similarity between a resume and a job description.</p>
        </div>
      </div>
      ...
      <div className="two-col" style={{ alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <div className="card-subtitle">Select a resume and a job to compare</div>
          ...
          <form onSubmit={handleMatch}>
            <div className="form-group">
              ...
              {resumes.length > 0 ? (
                ...
                <input className="form-input" type="number" min="1" placeholder="Enter resume ID" value={resumeId} onChange={e => setResumeId(e.target.value)} />
              )}
            </div>
            ...
            <div className="form-group">
              ...
              {jobs.length > 0 ? (
                ...
                <input className="form-input" type="number" min="1" placeholder="Enter job ID" value={jobId} onChange={e => setJobId(e.target.value)} />
              )}
            </div>
            ...
            <button type="submit" className="btn btn-primary btn-full" disabled={loading}>
              {loading
                ? <><span className="spinner" style={{ width: 15, height: 15, borderWidth: 2 }} /> Computing…</>
                : '⚡ Compute Match Score'}
            </button>
          </form>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 2.2 }}>
            ...
            <div>👍 <strong style={{ color: 'var(--primary)' }}>60–79</strong> — Good match</div>
        ...
        <div className="card">
          ...
          {!result && !loading && (
            <div className="empty-state">
              ...
              <p>Select resume and job, then click Compute.</p>
            </div>
          )}

          {loading && (
            <div className="spinner-wrap">
              <div className="spinner" />Running TF-IDF analysis…
            </div>
          )}
            ...
            <div className="fade-up">
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                <div
                  ...
                  style={{ '--score-color': meta.color, '--score-pct': `${pct}%` }}
                >
                  <span className="score-number">{result.score.toFixed(1)}</span>
                  <span className="score-unit">/ 100</span>
                </div>
                <div className="score-label" style={{ color: meta.color }}>{meta.label}</div>
                <div className="score-sub">Resume #{result.resume_id} vs Job #{result.job_id}</div>
              </div>

              <div className="score-bar-wrap">
                <div className="score-bar" style={{ width: `${pct}%`, background: meta.color }} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 13 }}>
                ...
                <InfoRow label="Computed at" value={new Date(result.created_at).toLocaleTimeString()} full />
              </div>
            </div>
          )}
        </div>
